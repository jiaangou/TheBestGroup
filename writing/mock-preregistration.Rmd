...
```
Whole-lake ecosystem processes are complex and hard to quantify. This is because ecosystem processes such as carbon and nitrogen cycling are determined by the composition and abundance of aquatic organisms, which themselves are highly heterogeneous and spatiotemporally-structured by a multitude of physical and chemical factors. Although studies that examine whole-lake ecosystem processes have focused primarily on pelagic production, @gushulak_effects_2021 showed, using stable isotope and nutrient concentration profiles of $\delta^{13}C$ and $\delta^{15}N$, that phytobenthic production and community composition also varies with lake depth. In this study, our objective is to replicate their study to test the robustness of their results and the validity of their approach.
...
```
We hypothesize that stable isotope signatures ( $\delta^{13}C$ and $\delta^{15}N$) and nutrient profiles (carbon and nitrogen concentrations, C/N ratio) varies with water depth.
...
The proposed study is an observational study. Surface layer soil samples will be collected from various transects of a lake bed. A number of analyses will be performed on these samples. For further details see the sampling plan and description of variables below.
...
Personnel who collect samples will be aware of the location, and depth the samples were collected at. Each sample will be labeled with a number so when the variables are measured, personnel will not be aware of the location or depth of the samples.
...
```
This study is conducted as a correlational study design. We are interesting in discovering how our variables (Stable isotopic ratio of nitrogen (N) and carbon(C); C:N ratio; %N content; %C content) change over a depth gradient (1 - 16 m).
...
```
Data will be collected at the Gall Lake study site ($50^{o} 11''$ N, $90^{o} 42''$ W) located in Ontario, Canada.This undisturbed study site is surrounded by boreal forest with a high abundance of black spruce, jack pine, and poplar and a lower abundance of birch, balsam fir, and larch. \@kingsbury2012 have described the physical and chemical characteristics of Gall Lake. Data collection will take place in the summers of 2022, 2023, and 2024.

Surface sediment samples (0-1cm) will be collected using a mini-Glew gravity coring apparatus [@glew1991] as was done in @gushulak2021 at sequential depht intervals, determined using depth sounders [@gushulak2021]. The surface sediment sampling will be along three depth transects in each of the basins of Gall Lake, to expand upon the area covered in @gushulak2021 and account for any differences in deposition to the surface sediments due to currents, input, and mixing in Gall Lake. This expansion of coverage will allow for spatial variation among basins, as well as for variation within basins, to be considered in the geochemical analysis of the replication study.

The stable isotope analysis from the surface sediment samples will be conducted using standard methods [@savage2004; @bunting2010] using Thermo Finnigan Delta V isotope ratio mass spectrometer that is equipped with a ConFlow IV dilution inlet system, to ensure consistency with the methods of @gushulak2021.
...
```
We will sample each 1m depth interval (e.g. 1-2m, 2-3m, 3-4m, etc.) until we reach the deepest point of Gall Lake (\~17m; [@kingsbury2012]).
